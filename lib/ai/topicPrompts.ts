...
export function buildFoodDescriptionPrompt(
  ...
  const ctx = goalRuSport(goal);
  return `Ты — нутрициолог внутри приложения LevelUp. Пользователь описал приём пищи: «${description}».
Контекст цели: ${ctx}.
...
export function buildFoodImagePrompt(_topicId: string, goal: BodyGoal): string {
  const ctx = goalRuSport(goal);
  return `Ты — нутрициолог внутри приложения LevelUp. На фото еда. Контекст цели: ${ctx}.
